...
def get_dependant(
    *,
    ...
    endpoint_signature = get_typed_signature(
        call=call,
        callable_info_cache=callable_info_cache,
    )
...
async def solve_generator(
    *,
    ...
    if is_gen_callable(call=call, callable_info_cache=callable_info_cache):
        cm = contextmanager_in_threadpool(contextmanager(call)(**sub_values))
    elif is_async_gen_callable(call=call, callable_info_cache=callable_info_cache):
...
async def solve_dependencies(
    *,
    ...
    for sub_dependant in dependant.dependencies:
        ...
        elif is_gen_callable(
            call=call,
            callable_info_cache=callable_info_cache,
        ) or is_async_gen_callable(
            call=call,
            callable_info_cache=callable_info_cache,
        ):
...
def init_callable_info_cache() -> CallableInfoCache:
...
def get_typed_signature(
    ...
    callable_info = _get_cached_callable_info(
        call=call,
        callable_info_cache=callable_info_cache,
    )
    return callable_info.typed_signature
...
def is_gen_callable(
    ...
    return callable_info.is_gen_callable


def is_async_gen_callable(
    call: Callable[..., Any],
    callable_info_cache: Optional[CallableInfoCache] = None,
) -> bool:
    callable_info = _get_cached_callable_info(
    ...
    return callable_info.is_async_gen_callable


def is_coroutine_callable(
    call: Callable[..., Any],
    callable_info_cache: Optional[CallableInfoCache] = None,
) -> bool:
    callable_info = _get_cached_callable_info(
    ...
    return callable_info.is_coroutine_callable


def _inspect_callable(call: Callable[..., Any]) -> CallableInfo:
    return CallableInfo(
        typed_signature=_get_typed_signature(call),
        is_gen_callable=_is_gen_callable(call),
        is_async_gen_callable=_is_async_gen_callable(call),
        is_coroutine_callable=_is_coroutine_callable(call),
    )


def _get_cached_callable_info(
    call: Callable[..., Any],
    callable_info_cache: Optional[CallableInfoCache] = None,
) -> CallableInfo:
    if callable_info_cache is not None:
        try:
            callable_info = callable_info_cache.get(call)
        except TypeError:
            # cannot create weakref, don't add to cache
            return _inspect_callable(call)
    else:
        callable_info = None

    # If not cached
    if callable_info is None:
        callable_info = _inspect_callable(call)
        if callable_info_cache is not None:
